...
import xgboost as xgb # ¡Importante!
...
# --- 4. Carga y Preparación de Datos (¡TODOS LOS DATOS AHORA!) ---
print("Iniciando carga de datos...")
df_raw = pd.read_csv(csv_file)
# --- ¡ARREGLO PARA EL 'KeyError'! ---
df_raw.columns = df_raw.columns.str.strip()
df_raw.columns = df_raw.columns.str.upper()

# --- ¡NUEVO BLOQUE PARA TABLA DE NULOS! ---
print("Generando tabla de nulos...")
...
print("Generando tablas descriptivas...")
...
# --- 4i. ¡NUEVO! Tabla de Métricas (Pestaña 7.d) ---
print("Generando tabla de métricas...")
data_metrics_xgb = {
...
COLORS = {
    ...
    'border': '#E2E8F0',
    'sarima': '#EF553B',   # Rojo para SARIMA
    'xgboost': '#00CC96'   # Verde para XGBoost
}
...
# --- 6. Creación de Gráficos (¡AHORA INCLUYE MODELO!) ---
print("Generando figuras EDA...")
# (Se colapsa 6a y 6b por brevedad, no cambian)
...
print("¡Figuras de EDA listas!")

# --- 6c. Figuras para Pestaña 7.c (Visualización del Modelo) ---
print("Generando figuras de Modelo...")
# (Se colapsa la lógica del modelo por brevedad, no cambia)
...
# --- ¡CAMBIO 2: GRÁFICO DE COMPARACIÓN DE MODELOS! ---
print("Generando gráfico comparativo de modelos...")
# Datos manuales extraídos de tus imágenes
models_data = {
...
# --- 6d. Aplicar estilo de fondo y fuente a TODOS los gráficos ---
# (Código de estilización de figuras - sin cambios)
for fig in [fig_top_deptos, fig_zona, fig_sexo, fig_zona_sexo,
            fig_prediccion, fig_residuales,
            fig_deco_obs, fig_deco_trend, fig_deco_seas, fig_deco_resid,
            fig_model_compare]: # <-- ¡CAMBIO 3: AÑADIDA FIGURA DE COMPARACIÓN!
    fig.update_layout(
...
print("¡Todas las figuras listas! Definiendo layout...")
...
app.layout = html.Div([
    ...
    dcc.Tabs(id="tabs-principales", value='tab-7', style=tabs_styles, children=[
        ...
        # Pestañas 1-6
        # (Código de pestañas 1-6 - sin cambios)
                ...
                dcc.Tabs(id="tabs-metodologia", value='sub-tab-6a', children=[
                    ...
                    dcc.Tab(label='a. Definición del Problema', value='sub-tab-6a', style=tab_style, selected_style=tab_selected_style, children=[
                        html.Div(style={'padding': '20px'}, children=[
                            ...
                            html.Hr(style={'margin': '20px 0'}), # Añadimos un separador

                            # Usamos el estilo 'secondary' que ya tienes definido
                            ...
                            # Usamos una lista para detallar el alcance
                            html.Ul([
                    ...
                    dcc.Tab(label='b. Preparación de los Datos', value='sub-tab-6b', style=tab_style, selected_style=tab_selected_style, children=[
                        html.Div(style={'padding': '20px'}, children=[
                            ...
                            dash_table.DataTable(
                                ...
                                style_table={**table_style, 'maxWidth': '500px', 'margin': 'auto'}, # Estilo centrado
...
""", style={'fontFamily': 'monospace'}), style={'backgroundColor': '#F8FAFC', 'padding': '10px', 'borderRadius': '5px', 'border': f'1px solid {COLORS["border"]}', 'overflowX': 'auto'}),
                    ...
                    # --- ¡CAMBIO 4: REEMPLAZO DE LA SUB-PESTAÑA 6.c! ---
                    dcc.Tab(label='c. Selección del Modelo', value='sub-tab-6c', style=tab_style, selected_style=tab_selected_style, children=[
                        html.Div(style={'padding': '20px'}, children=[
                            ...
                            # Ecuación SARIMA (representación textual estilizada)
                            ...
                            # Ecuación XGBoost (representación textual estilizada)
                            ...
                            dcc.Graph(figure=fig_model_compare), # Aquí mostramos el gráfico comparativo
        ...
        dcc.Tab(label='7. Resultados y análisis final', value='tab-7', style=tab_style, selected_style=tab_selected_style, children=[
            html.Div(style={'padding': '20px'}, children=[
                ...
                dcc.Tabs(id="tabs-anidadas", value='sub-tab-d', children=[ 
                    ...
                    # --- Sub-Pestaña a: EDA 1 (sin cambios) ---
                    dcc.Tab(label='a. EDA 1', value='sub-tab-a', style=tab_style, selected_style=tab_selected_style, children=[
                        html.Div(style={'padding': '20px'}, children=[
                            ...
                            html.Div([
                                html.Div([
                                    html.H4("Total Homicidios", style=kpi_title_style),
                                    html.P(f"{total_homicidios:,.0f}", style=kpi_value_style)
                                ], style=kpi_card_style),
                                html.Div([
                                    html.H4("Fecha Inicio", style=kpi_title_style),
                                    html.P(fecha_inicio, style=kpi_value_style)
                                ], style=kpi_card_style),
                                html.Div([
                                    html.H4("Fecha Fin", style=kpi_title_style),
                                    html.P(fecha_fin, style=kpi_value_style)
                                ], style=kpi_card_style),
                            ], style=kpi_row_style), 
                            html.Hr(),
                    ...
                    # --- Sub-Pestaña b: EDA 2 (sin cambios) ---
                    dcc.Tab(label='b. EDA 2', value='sub-tab-b', style=tab_style, selected_style=tab_selected_style, children=[
                        html.Div(style={'padding': '20px'}, children=[
                            ...
                            html.Div([
                                html.Div([
                                    html.H4("Depto. con MÁS Homicidios", style=kpi_title_style_small),
                                    html.P(f"{kpi_depto_max_nombre} ({kpi_depto_max_valor})", style=kpi_value_style_small)
                                ], style=kpi_card_style_2_col),
                                html.Div([
                                    html.H4("Depto. con MENOS Homicidios", style=kpi_title_style_small),
                                    html.P(f"{kpi_depto_min_nombre} ({kpi_depto_min_valor})", style=kpi_value_style_small)
                                ], style=kpi_card_style_2_col),
                            ], style=kpi_row_style), 
                            html.Hr(), 
                    ...
                    # --- Sub-Pestaña c: Visualización (sin cambios) ---
                    dcc.Tab(label='c. Visualización del modelo', value='sub-tab-c', style=tab_style, selected_style=tab_selected_style, children=[
                        html.Div(style={'padding': '20px'}, children=[
                            ...
                            html.Div([
                                html.Div([
                                    html.H4("Inicio del Pronóstico", style=kpi_title_style_small),
                                    html.P("Octubre 2025", style=kpi_value_style_small)
                                ], style=kpi_card_style_2_col),
                                html.Div([
                                    html.H4("Fin del Pronóstico", style=kpi_title_style_small),
                                    html.P("Diciembre 2025", style=kpi_value_style_small)
                                ], style=kpi_card_style_2_col),
                            ], style=kpi_row_style),
                            html.Hr(),
                    ...
                    # --- Sub-Pestaña d: Indicadores (sin cambios) ---
                    ...
                    # --- Sub-Pestaña e: Limitaciones (Ya actualizada) ---
